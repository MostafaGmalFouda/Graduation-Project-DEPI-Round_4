...
// ====================== AUTOMATIC DASHBOARD ======================
function generateAutomaticDashboard() {
    const formData = new FormData();
    formData.append('chart_type', 'automatic_dashboard');

    // Show loading state
    const autoContainer = document.getElementById('result-automatic');
    autoContainer.innerHTML = `
        <div style="text-align:center; padding:40px;">
            <i class="fas fa-spinner fa-spin" style="font-size:42px; color:#a855f7;"></i>
            <p style="margin-top:20px; color:#a855f7;">Generating Complete Automatic Dashboard...</p>
        </div>`;

    fetch('/phase2/generate', {
        method: 'POST',
        body: formData
    })
        ...
        .then(data => {
            if (data.status === "success") {
                autoContainer.innerHTML = `
                <div class="viz-results-area">
                    <h3 style="text-align:center; margin-bottom:15px; color:#06d49d;">
                        📊 Automatic EDA Dashboard
                    </h3>
                    <iframe src="${data.view_url}" 
                            style="width:100%; height:750px; border:none; border-radius:12px; background:#0b0b18;" 
                            allowfullscreen></iframe>
                </div>`;
            } else {
                autoContainer.innerHTML = `<p style="color:red; text-align:center;">Error: ${data.message}</p>`;
            }
        })
        .catch(err => {
            console.error(err);
            autoContainer.innerHTML = `<p style="color:red; text-align:center;">Failed to generate dashboard</p>`;
        });
}